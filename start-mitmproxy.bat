...
rem Enable proxy authentication when a password is configured.
set "EXTRA_ARGS="
set "AUTH_STATUS=disabled"
if not "%MITM_PROXY_PASS%"=="" (
  set "EXTRA_ARGS=--set proxyauth=%MITM_PROXY_USER%:%MITM_PROXY_PASS%"
...
if /i "%MODE%"=="web" (
  set "APP_CMD=mitmweb --web-host 0.0.0.0 --web-port %WEB_PORT% --listen-port %PROXY_PORT%"
  set "INTERACTIVE=-d"
